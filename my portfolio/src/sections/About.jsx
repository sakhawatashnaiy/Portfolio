...
export default function AboutSection() {
	...
	return (
		<section
			id="about"
			className="relative py-16 sm:py-20 lg:py-24 text-slate-700 dark:text-neutral-300 bg-slate-50 dark:[background:radial-gradient(circle_at_top,rgba(15,118,255,0.12),transparent_45%),radial-gradient(circle_at_bottom,rgba(14,165,233,0.08),transparent_55%),#0a0a0a] transition-colors"
		>
			<div className="max-w-7xl mx-auto px-3 sm:px-5 lg:px-12">
				...
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-start lg:items-center max-w-5xl mx-auto text-center lg:text-left">
					...
					<div className="lg:col-span-9 space-y-4 sm:space-y-6 justify-center">
						...
						<p className="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-slate-900 dark:text-white text-balance">
							Building confident interfaces and systems that feel inevitable.
						</p>
						<p className="text-base sm:text-xl text-slate-700 dark:text-neutral-200/90 text-balance">
						...
						<div className="space-y-3 sm:space-y-4 text-sm sm:text-base leading-relaxed text-slate-600 dark:text-neutral-300/90">
							<p>
						...
						<button
							...
							className="group inline-flex w-full sm:w-auto justify-center items-center gap-2 rounded-full border border-slate-300/80 dark:border-neutral-800 bg-white/70 dark:bg-neutral-900/60 px-5 py-2 text-sm font-semibold text-slate-900 dark:text-white transition-all duration-500 ease-[cubic-bezier(0.65,_0,_0.35,_1)] hover:border-indigo-400/60 hover:bg-white dark:hover:bg-neutral-900"
						>
				...
				{/* Timeline */}
				<section className="mt-16 sm:mt-24 space-y-8 sm:space-y-10 text-slate-900 dark:text-white sm:italic">
					<header className="text-center">
						<p className="text-xl sm:text-4xl uppercase tracking-[0.22em] sm:tracking-[0.35em] text-slate-900 dark:text-white">Trajectory</p>
						<h3 className="mt-2 sm:mt-3 text-2xl sm:text-4xl font-bold tracking-tight text-slate-900 dark:text-white text-balance">
					...
					<div className="relative max-w-3xl mx-auto">
						<span
							className="hidden sm:block absolute top-0 bottom-0 w-px bg-slate-300 dark:bg-white left-4 sm:left-1/2 sm:-translate-x-1/2"
							aria-hidden
						/>
						...
						<div className="space-y-6 sm:space-y-10">
							{timeline.map((item) => (
								...
								>
											<span className="hidden sm:block absolute left-4 top-0 h-3 w-3 sm:h-4 sm:w-4 rounded-full border border-slate-300 dark:border-white bg-slate-300 dark:bg-white transition-all duration-500 sm:left-1/2 sm:-translate-x-1/2" />
									<div className="transition-all duration-500">
												<div className="rounded-2xl border border-slate-200/70 dark:border-white/5 bg-white/70 dark:bg-neutral-900/30 backdrop-blur-md p-4 sm:p-6 shadow-lg text-center transition-colors">
													<p className="text-sm uppercase tracking-[0.25em] sm:tracking-[0.4em] text-slate-700 dark:text-white">{item.year}</p>
													<h4 className="mt-2 sm:mt-3 text-xl sm:text-3xl font-semibold text-slate-900 dark:text-white">{item.title}</h4>
													<p className="mt-2 text-base sm:text-lg leading-relaxed text-slate-700 dark:text-white/90">{item.desc}</p>
										</div>
									</div>
								</article>
							))}
				...
				<section className="mt-16 sm:mt-24">
					<header className="space-y-2 text-center">
						...
						<h3 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white text-balance">Technical capabilities</h3>
						<p className="text-sm sm:text-base text-slate-500 dark:text-neutral-500">The tools and platforms I use to deliver reliable, expressive interfaces.</p>
					</header>
